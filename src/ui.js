...
function renderReport() {
  ...
  parts.push(actionsHTML());       // PDF 저장·입력 수정·초기화
  parts.push(caseCtaHTML('main')); // 내 사례 남기기
  parts.push(ytCtaHTML());         // 유튜브 영상
  parts.push(ctaHTML('main'));     // 오류·개선 요청하기
  parts.push(basisHTML(ctx));      // 근거와 한계 — 제일 마지막
...
/* 결과 액션 — PDF 저장을 별도 영역으로 (하단 순서: PDF → 내 사례 → 유튜브 → 오류·개선 → 근거) */
function actionsHTML() {
  return `<div class="card no-print">
    <h2>결과 저장</h2>
    <p class="hint">지금 화면의 전체 결과(심층 분석 포함)를 PDF 파일로 저장합니다.</p>
    <div class="chiprow" style="display:flex;gap:8px;flex-wrap:wrap">
      <button class="btn primary" id="btnPrint" style="flex:none">결과 PDF 저장</button>
...
function caseCtaHTML(kind) {
  ...
  if (kind === 'main') return `<div class="cta">
    <h3>내 경우에는 어떻게 하는 게 좋을까요?</h3>
    <p>계산 결과를 보다가 궁금한 점이 생겼다면 현재 상황과 고민을 남겨주세요.<br>
    비슷한 고민이 많거나 도움이 될 만한 사례는 닥터마빈이 직접 검토해 콘텐츠로 답변드립니다.</p>
    ...
    <p class="fine">※ 모든 사연에 개별 답변드리지는 않습니다.<br>※ 계산기에 입력한 내용은 자동으로 전송되지 않습니다.</p>
  </div>`;
