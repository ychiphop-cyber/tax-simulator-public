...
/* ═══════════════════════════════════════════════════════════════════
   ...
   [2026-09-02 갱신] 2026.9.1 국무회의 수정 정부안 반영. 바뀐 셀은 종부세 정부안(reform)
   항목뿐이며, 각 셀의 옛 값은 주석으로 남긴다. 갱신값은 test/gen_expected.js(reference.js
   법정 산식, 엔진 미사용)로 산출했다. 현행(current) 셀·재산세·양도세·증여·취득세는 불변.
   - GC-1(비거주 부부 공동명의): 개별납부 공제 각 4억→6억, 특례 공제 9억→12억
   - GC-2(실거주 단독 45억): 기본공제 14억 불변, 세부담상한 200%→150%로 2027·2028 정부안 축소
   - GC-2 전환 분석: 전환 후 개별납부 공제 각 9억(실거주, 불변)이지만 전환 전 단독 세액이
     상한 150%로 낮아져 연평균 절감 458.6만→305.8만
   ═══════════════════════════════════════════════════════════════════ */
...
console.log('\nGC-1 비거주 1주택 · 부부 공동명의 50/50 · 공시 25.6억');
{
  ...
  T('2027 정부안 1,193.7만 (9·1: 개별납부 각 6억·특례 12억 / 옛 값 1,435.8만)', ref[1].holdTax, 1193.7);
  T('2028 정부안 1,333.3만 (옛 값 1,659.7만)', ref[2].holdTax, 1333.3);
  T('2029 정부안 1,333.3만 (옛 값 1,659.7만)', ref[3].holdTax, 1333.3);
  T('2030 정부안 1,333.3만 (옛 값 1,659.7만)', ref[4].holdTax, 1333.3);
  const j = ref[2].jong;
  TB('2028 joint-compare 모드', j.mode === 'joint-compare', j.mode);
  const me = j.joint.indiv[0].r;
  T('  개별납부(1인) 과표 5억4,400만 = (12.8억 − 6억) × 80% (옛 값 7억400만 = 4억 공제)', me.base, 54400);
  T('  개별납부(1인) 산출세액 320.8만 (옛 값 495만)', me.gross, 320.8);
  T('  개별납부(1인) 재산세공제 130.6만 (옛 값 169만)', me.propCredit, 130.6);
  T('  개별납부(1인) 합계 228.3만 (옛 값 391만)', me.total, 228.3);
  TB('  개별납부 기본공제 6억(비거주, 9·1 수정 — 옛 4억)', me.deduct === 6 * 억, E.won(me.deduct));
  TB('  개별납부 납세자 유형 jointOneIndiv (일반 다주택과 분리)', me.taxpayerType === 'jointOneIndiv', me.taxpayerType);
  TB('  개별납부 FMV 80%', Math.abs(me.fair - 0.80) < 1e-9, me.fair);
  T('  부부 개별 합산 456.6만 (옛 값 783만)', j.joint.indivTotal, 456.6);
  const sp = j.joint.special;
  T('  특례 과표 9억5,200만 = (25.6억 − 12억) × 70% (옛 값 11억6,200만 = 9억 공제)', sp.base, 95200);
  T('  특례 합계 706.9만 (옛 값 974만)', sp.total, 706.9);
  TB('  특례 기본공제 12억(비거주, 9·1 수정 — 옛 9억)', sp.deduct === 12 * 억, E.won(sp.deduct));
...
console.log('\nGC-2 초고가 1주택 · 단독 · 공시 45억 · 66세/거주 20년 · 공동명의 전환');
{
  ...
  T('2027 정부안 2,304.9만 (9·1: 세부담상한 150% / 옛 값 2,947.3만 = 상한 200%)', ref[1].holdTax, 2304.9);
  T('2028 정부안 3,268.4만 (옛 값 3,949.4만)', ref[2].holdTax, 3268.4);
  ...
  T('  종부세 합계 1,670만 — 상한 150% 적용 (옛 값 2,351만)', p.total, 1670);
  TB('  2028 세부담상한 발동(150%)', p.capped > 0, String(p.capped));
  ...
  TB('전환: 연평균 절감 305.8만 내외 (옛 값 458.6만 — 단독 세액이 상한 150%로 낮아진 결과)', Math.abs(jc.annual - 305.8 * 만) <= TOL, (jc.annual / 만).toFixed(1) + '만');
