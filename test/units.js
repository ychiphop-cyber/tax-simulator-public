...
console.log('\nT-002b 파이프라인 검증 — 2026년 58세·거주 8년 시작 → 2028년 60세·10년');
{
  ...
  const ref = E.holdSim(inp, 'reform');
  const r28 = ref[2];
  // r2/r3 반영: 재산세 과표상한 5% 체이닝(P0-3B) + 2027년 이후 1주택 공정시장가액비율 특례 60% 복귀 가정(P0-3A')으로
  // 파이프라인 수치가 공식사례 단순계산(353.7만)과 달라짐 — 단위 검증(T-002)은 그대로 유지됨
  T('2028 종부세 ≈ 8.74만 (r3 파이프라인)', approx(r28.jong.total, 87360, 500), `got ${r28.jong.total}`);
  T('2028 보유세 ≈ 365.24만 (r3 파이프라인)', approx(r28.holdTax, 3652373, 1500), `got ${r28.holdTax}`);
}
